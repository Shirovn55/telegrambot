...
BROADCAST_COOLDOWN = 30  # seconds - chỉ cho phép broadcast mỗi 30s
    ...
    if text and text.startswith("/thongbao"):
        ...
        if LAST_BROADCAST_TIME:
            ...
            if time_since_last < BROADCAST_COOLDOWN:
                ...
                tg_send(
                    chat_id,
                    f"⏳ <b>Vui lòng đợi {wait_time}s trước khi gửi broadcast tiếp</b>\n\n"
                    f"Điều này giúp tránh spam và bảo vệ bot."
                )
                return
        ...
        if IS_BROADCASTING:
            tg_send(
                chat_id,
                "⏳ <b>Đang có broadcast khác đang chạy</b>\n\n"
